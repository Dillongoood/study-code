...
def make_file_name(stn_id, product, valid_time):
    ...
    file_name = f"{stn_id}_{ymd}{hms}{product}"
    return file_name
...
def main():
    ...
    while start_accum < end_time:
        ...
        # start the loop over the 6 min files in this accumulation period
        while in_time <= end_accum:
            ...
            if count == 0:
                ...
                # scan_time_dt = datetime.datetime.strptime(ds_in.scan_time, '%Y-%m-%d %H:%M:%S')
                # start_accum_time = int(scan_time_dt.strftime('%Y%m%d%H%M%S'))
        ...
        if has_data:
            ...
            # my_meta_data['variable'] = "HSR"
            # my_meta_data["product"] = config["OUT_PRODUCT"]

            my_meta_data = {}
            my_meta_data["station_id"] = str(ds_out.__getattr__("site_code"))
            my_meta_data["station_name"] = str(ds_out.__getattr__("site_name"))
            # my_meta_data['valid_time'] = int(ds_out['valid_time'][0].item())
            # my_meta_data['start_time'] = int(ds_out['start_time'][0].item())
            my_meta_data['variable'] = "HSR"
            my_meta_data["product"] = config["OUT_PRODUCT"]
